---
title: "Hub para el Fortalecimiento en Comunicación y Visualización de Datos"
output: html_document
---


<style type="text/css">

/* Navbar Styles */
.navbar {
  background-color: #CD8C95 !important; /* Background color of the navbar */
  border-color: #8B5F65 !important; /* Border color of the navbar */
  background-image: none !important; /* Removes any background image */
  height: 65px; /* Sets navbar height */
}

/* Navbar Link Styles */
.navbar-default .navbar-nav > li > a {
  color: #FFFFFF !important;  /* Text color for links */
  font-weight: bold !important; /* Bold text */
}

.navbar-default .navbar-nav > li > a:hover,
.navbar-default .navbar-nav > li > a:focus {
  color: #8B5F65 !important; /* Hover and focus text color */
  font-weight: bold; /* Keeps text bold on hover/focus */
}

/* Navbar Collapse Menu Styles */
.navbar-collapse {
  background-color: #CD8C95 !important; /* Background color for the collapsed navbar */
}

/* Dropdown Menu Link Styles */
.navbar-nav > li > a {
  color: #FFFFFF; /* Text color for dropdown links */
  font-weight: bold;
  padding: 10px 15px; /* Padding for dropdown links */
}

.navbar-nav > li > a:hover,
.navbar-nav > li > a:focus,
.navbar-nav > .active > a {
  background-color: #005a9c; /* Background color on hover/active */
  color: #FFFFFF; /* Text color on hover/active */
}

/* Active List Group Item Styles */
.list-group-item.active,
.list-group-item.active:focus,
.list-group-item.active:hover {
  z-index: 2;
  color: #8B5F65; /* Text color for active list item */
  background-color: #EEDFCC; /* Background color for active list item */
  border-color: #8B5F65; /* Border color for active list item */
}

/* Title, Subtitle, Author, and Date Styles */
h1.title {
  color: #8B5F65; /* Color for title */
}

h4.subtitle {
  color: #8B5F65; /* Color for subtitle */
}

h5.author {
  color: #8B5F65; /* Color for author */
}

h5.date {
  color: #8B5F65; /* Color for date */
}

/* Hyperlink Styles */
a {
  color: #582C35; /* Text color for hyperlinks */
  text-decoration: none; /* Removes underline from hyperlinks */
}

a:hover {
  color: #005a9c; /* Color when hovering over hyperlinks */
  text-decoration: underline; /* Underlines text when hovering over hyperlinks */
}

a:visited {
  color: #6C4F56; /* Color for visited hyperlinks */
}

a:active {
  color: #4A2C31; /* Color for active hyperlinks (while clicking) */
}

</style>



<div class="row" style="padding-top: 30px;">
<div class="col-sm-6" style="color:#8B5F65;">

# **Portal de Fortalecimiento y actualización de habilidades de comunicación y visualización de datos**

Este sitio web nace como una **herramienta interna colaborativa**, diseñada para fortalecer las capacidades del equipo en la **comunicación, visualización y ciudadanización de las estadísticas oficiales de gobierno**.  

Inspirado en diversas iniciativas, este espacio busca crear una **comunidad activa y comprometida** con el aprendizaje, la actualización constante y la mejora de la difusión de datos hacia la sociedad.  . *Este sitio web se encuentra actualmente en desarrollo.*

## 📌 **Nuestra Visión**  

El **Portal de Fortalecimiento Estadístico** está construido sobre los valores fundamentales de:  

- **Accesibilidad**  
- **Soporte mutuo**  
- **Estructura organizada**  
- **Colaboración continua**  

Aspiramos a fomentar un entorno donde el conocimiento **fluya libremente**, y donde cada integrante del equipo pueda crecer y contribuir al desarrollo de **estadísticas más claras, accesibles y útiles** para la ciudadanía.  


## 🚀 **Qué Ofrecemos**  

Este espacio incluye **recursos, cursos, guías y herramientas clave** para mejorar en:  

✔ **Redacción académica y comunicación estadística efectiva**  
✔ **Visualización de datos comprensible y accesible**  
✔ **Ciudadanización de las estadísticas**, promoviendo un lenguaje claro y cercano  
✔ **Aprovechamiento del código abierto**, fomentando la transparencia y el intercambio de buenas prácticas  


## 🌎 **Comunidad y Colaboración**  

En línea con el espíritu de *Escuela de Datos*, este portal es más que una plataforma de aprendizaje: es un **espacio de socialización e inspiración**.  

Aquí, el compromiso con el equipo y con la sociedad se traduce en la **creación de contenidos estadísticos más relevantes y accesibles**, para contribuir al cambio social a través del conocimiento.  



## 🎯 **Ventajas de Participar**  

Al unirte a esta plataforma, accedes a:  

📚 **Recursos y soporte continuo** para tus proyectos y publicaciones  
📊 **Capacitación en herramientas de comunicación y visualización de datos**  
🤝 **Acceso a una comunidad de aprendizaje**, donde podrás compartir experiencias, resolver dudas y fortalecer tus habilidades  
💡 **Inspiración y nuevas ideas** para mejorar la difusión de los resultados estadísticos  



Este portal representa nuestro compromiso con el **fortalecimiento de las estadísticas oficiales**, con el propósito de hacerlas más cercanas, útiles y significativas para la ciudadanía.  

> **Juntos, construiremos un puente entre el conocimiento técnico y el bienestar social.**  

Disfruta y envía recomendaciones a este espacio


</div>
<div class="col-sm-6">

<!-- ### [Crump Lab: Human Cognition and Performance](https://crumplab.github.io) -->

<!-- <a href = "https://crumplab.github.io"> -->
![](images/logo.png)
</a>
</div>
